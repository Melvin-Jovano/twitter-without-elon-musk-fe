...
<script setup>
    ...
    async function scrolledToBottom(div) {
        if(div.scrollTop + div.clientHeight >= div.scrollHeight && data.lastPostId !== null) {
            const getPosts = await getAllPostsById({ lastId: data.lastPostId });
            if (getPosts.data.message === 'SUCCESS') {
                data.tweetData.push(...getPosts.data.data.data);
                data.lastPostId = getPosts.data.data.lastId;
            }
        }
    }
    ...
    async function getPosts(){
        try {
            ...
            if(posts.data.message === "SUCCESS"){
                data.tweetsCount = posts.data.data.data.length;
                data.tweetData = posts.data.data.data;
                data.lastPostId = posts.data.data.lastId;
            }
        } catch (error) {
            return;
        }
